...
biplot(pca.iris, cex = 0) # show loading

# make a better variable loading plot
library(ggfortify)
autoplot(pca.iris, 
         data = iris, 
         colour = 'Species', 
...
## 10. Case Study: Transcriptome analysis of Borrelia growth
...
The original paper is available at rom [Arnold et al (2016), PLoS One](https://journals.plos.org/plosone/article?id=10.1371/journal.pone.0164165). 
...
# scale by sample:
pheatmap(log2(bb.sig[,3:11]+1), scale = "column", labels_row = bb.sig$locus)
...
pheatmap(log2(bb.sig[,3:11]+1), scale = "row", labels_row = bb.sig$locus, cellwidth = 30)
...
## IGRORE BELOW
### Identify genes differentially expressed between 3 phases
Need ANOVA, which test the null hypothesis that within-group variance  = between-group variance
...
anova.out <- bb.long %>% 
  group_by(locus) %>% 
  nest() |> # use nested table
  mutate(model = map(data, ~lm(data = .x, log2.counts ~ growth.phase)),
         summary = map(model, summary))

# Tidy results
tidy_results <- anova.out %>% 
...
anova.out2 <- bb.long |>
  group_by(locus) |>
  do(tidy(lm(data = ., log2.counts ~ growth.phase)))
```


To get F stats, have to use the "for" loop or "lapply"

```{r}
f.stats <- function(x) {
  locus <- x$locus[1]
  lm.x <- lm(data = x, log2.counts ~ growth.phase)
  sum.lm <- summary(lm.x)
  fstat = sum.lm$fstatistic
  p.value = pf(q = fstat[1], df1 = fstat[2], df2 = fstat[3])
  return(list(loc = locus, p.value = p.value))
}

loci <- bb.long |> pull(locus) |> unique()

anova.results <- lapply(loci, function(x){
  locus.df <- bb.long |> filter(locus == x)
  f.stats(locus.df)
})

result.df <- bind_rows(anova.results)
...
Validate sig genes with boxplots and heatmap
